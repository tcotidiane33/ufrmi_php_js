...
if (isset($_POST['submit'])) {
	$numpan = $_POST['nupan'];
	// $codepan = $_POST['codepan'];
	$formatpan = $_POST['forpan'];
	$typepan = $_POST['typan'];
	$sitepan = $_POST['sipan'];
	$imgpan = $_POST['imgpan'];
	$etatpan = $_POST['etapan'];
	$prixpan = $_POST['pripan'];
	
	$query = "insert into  PANNEAU(NUMPAN,CODECOM,FORMATPAN,TYPEPAN,SITEPAN,IMAGEPAN,ETATPAN, PRIXUNITAIREPAN)
			 values(?,?,?,?,?,?,?)";
	$stmt = $mysqli->prepare($query);
	$rc = $stmt->bind_param($numpan,$codecom ,$formatpan, $typepan, $sitepan, $imgpan, $etatpan, $prixpan);
	$stmt->execute();
	echo "<script>alert('Panneau Succssfully register');</script>";
}
?>
								...
								<div class="panel panel-primary">
									...
									<div class="panel-body">
										<form method="post" action="" name="registration" class="form-horizontal" onSubmit="return valid();">
											...
											<div class="form-group">
												<label class="col-sm-2 control-label"> Image du panneau : </label>
												<div class="col-sm-8">
													<input type="file" name="imgpan" id="imgpan" class="form-control" required="required">
												</div>
											</div>
